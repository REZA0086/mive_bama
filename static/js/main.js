...
//========================================
$('.action-plus').on('click', function (event) {
...
//========================================
$('.action-minus').on('click', function (event) {
    ...
    if (decrementValue < 2) {
        $(`.${event.currentTarget.attributes[4].nodeValue}`).hide('slow')
    }
    let id = event.currentTarget.attributes[2].nodeValue;
    let code_id = event.currentTarget.attributes[3].nodeValue;

    $.get('Order/remove_order?product_id=' + id).then(res => {
        if (res.final_price) {
            document.querySelector("#" + code_id).innerHTML = res.final_price;
        }
    })
